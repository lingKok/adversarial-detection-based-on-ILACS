...
# from RawModels.ResNet import CIFAR10_Training_Parameters, adjust_learning_rate, resnet20_cifar,Net,Net2
from RawModels.ResNet_for_stl10 import STL10_Training_Parameters,resnet20_stl,adjust_learning_rate
...
from RawModels.Utils.dataset import get_stl10_train_validate_loader, get_stl10_test_loader
...
def main(args):
    ...
    train_loader, valid_loader = get_stl10_train_validate_loader(dir_name='./STL10/STL10/',
                                                                   batch_size=STL10_Training_Parameters['batch_size'],
                                                                   valid_size=0.1, shuffle=True)
    test_loader = get_stl10_test_loader(dir_name='./STL10/STL10/', batch_size=STL10_Training_Parameters['batch_size'])

    # set up the model and optimizer
    resnet_model = resnet20_stl().to(device)

    optimizer = optim.Adam(resnet_model.parameters(), lr=STL10_Training_Parameters['lr'])

    # Training
    best_val_acc = None
    model_saver = './STL10/model/STL10_' + 'raw' + '.pt'
    # resnet_model.load(path=model_saver,device=device)
    for epoch in range(STL10_Training_Parameters['num_epochs']):
        # ct = 1
        ...
        # train_one_epoch_by_NCE_cifar(model=resnet_model,train_loader=train_loader,optimizer=optimizer,epoch=epoch,device=device)
        # validation
