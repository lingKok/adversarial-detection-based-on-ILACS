...
class JSMAAttack(Attack):
    ...
    def compute_jacobian(self, inputs, device):
        """
        ...
        for i in range(output.size()[1]):
            ...
            jacobian[i] = inputs.grad.squeeze().view(-1, num_features).clone()
            mask[:, i] = 0  # reset
            # print("3:{}".format(torch.cuda.memory_allocated(0)))
        output.backward(mask)
        del output, inputs, mask
        gc.collect()
    ...
    def perturbation_single(self, sample, ys_target, device):
        ...
        while (iter < max_iters) and (current[0] != ys_target[0]) and (search_domain.sum() != 0):
            ...
            # print("1:{}".format(torch.cuda.memory_allocated(0)))
            jacobian = self.compute_jacobian(inputs=var_sample, device=device)
            print("1:{}".format(torch.cuda.memory_allocated(0)))
            ...
            # apply modifications
            var_sample_flatten = var_sample.detach().view(-1, num_features)
            var_sample_flatten.data[0, p1] += self.theta
            var_sample_flatten.data[0, p2] += self.theta
            ...
            # var_sample = tensor2variable(torch.tensor(new_sample), device=device)
            output = self.model(var_sample)
            mask = torch.zeros(output.size()).to(device)
            mask[:, 0] = 1
            output.backward(mask)
            ...
            current = torch.max(output.data, 1)[1].cpu().numpy()
            iter += 1
        # var_sample = tensor2variable(torch.tensor(new_sample), device=device).detach()#free memory
        adv_samples = var_sample.detach().data.cpu().numpy()
        # print("1:{}".format(torch.cuda.memory_allocated(0)))
        del var_sample, jacobian, var_target, p1, p2, new_sample
        gc.collect()
        torch.cuda.empty_cache()
        print("3:{}".format(torch.cuda.memory_allocated(0)))
        return adv_samples
    ...
    def perturbation(self, xs, ys_target, device):
        ...
        for iter in range(len(xs)):
            try:
                print(iter)
                adv_image = self.perturbation_single(sample=xs[iter: iter + 1], ys_target=ys_target[iter: iter + 1],
                                                     device=device)
