...
class DistillationDefense(Defense):
    def __init__(self,
        ...
        :param device:
        """
        super(DistillationDefense, self).__init__(model=model,
        ...
        if self.Dataset == 'MNIST':
            ...
            self.initial_optimizer = optim.Adam(
                self.initial_model.parameters(), lr=training_parameters['lr'])
            self.distilled_optimizer = optim.Adam(
                self.distilled_model.parameters(),
    ...
    def train_initial_model_with_temperature(self,
                                             train_loader=None,
        ...
        for epoch in range(self.num_epochs):
            ...
            )  # set the model in the train mode before every epoch
            ...
            if self.Dataset == 'CIFAR10':
                adjust_learning_rate(optimizer=self.initial_optimizer,
                                     epoch=epoch)

            # save the initial model
            assert os.path.exists('./DefenseEnhancedModels/{}'.format(
                self.defense_name))
            initial_model_saver = './DefenseEnhancedModels/{}/{}_DD_initial.pt'.format(
                self.defense_name, self.Dataset)
    ...
    def train_distilled_model_with_temp(self,
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.Dataset == 'CIFAR10':
                adjust_learning_rate(optimizer=self.distilled_optimizer,
                                     epoch=epoch)

            # save the distilled model --> DD_enhanced model
            assert os.path.exists('./DefenseEnhancedModels/{}'.format(
                self.defense_name))
            distilled_model_saver = './DefenseEnhancedModels/{}/{}_{}_enhanced.pt'.format(
    ...
    def defense(self,
        ...
        if initial_flag is False:
            ...
            self.train_initial_model_with_temperature(
                train_loader=train_loader, validation_loader=validation_loader)

        # load the pre-trained initial model
        model_location = '../DefenseEnhancedModels/{}/{}_DD_initial.pt'.format(
            self.defense_name, self.Dataset)
        assert os.path.exists(
            model_location
        ), 'No initial model, please train the initial model first'
        ...
        with torch.no_grad():
            ...
            for images, _ in raw_train:
                ...
                initial_preds = F.softmax(initial_logits_temp,
        ...
        soft_dataset = SoftLabelDataset(images=torch.from_numpy(ori_images),
                                        labels=torch.from_numpy(soft_labels),
                                        dataset=self.Dataset,
                                        transform=self.transform)
        soft_dataset_loader = torch.utils.data.DataLoader(
            soft_dataset, batch_size=self.batch_size, shuffle=True)

        # train and save the distilled model -> DD_enhanced model
        self.train_distilled_model_with_temp(
            distilled_train_loader=soft_dataset_loader,
            validation_loader=validation_loader)
